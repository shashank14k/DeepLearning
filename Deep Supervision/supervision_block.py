...
class DeepSupervisionNetwork(tf.keras.Model):
    ...
    def train_start(self, train, epochs, train_len, batch_size, val, val_len,
                    learning_rate=1e-3, metrics=None, optimizer=None):

        if not optimizer:
            opt = Adam(learning_rate=learning_rate, epsilon=None, amsgrad=False,
